...
Code targets Go 1.26.1+ with `gofmt` and `goimports` enforcing formatting and import order. Package names remain lower-case without underscores; Go filenames prefer `snake_case.go` with tests co-located as `*_test.go`. Stick to idiomatic CamelCase for exported identifiers, provide godoc comments on exported types and functions, and keep configuration structures in YAML/JSON aligned with existing tags.
